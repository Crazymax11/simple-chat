...
class UsersStorage{
  ...
  createUser(values, callback){
    ...
    .then( (obj) => {return new Promise( (resolve, reject) => {
      //проверим существует ли такой юзер
      this._doesLoginExists({login: obj.login}, (ex) => ex? reject(new Error("such login already created")) : resolve(obj));
    })})
    .then( (obj) => {return new Promise( (resolve, reject) => {
      //проверим занят ли юзернейм
      this._doesUsernameExists({username: obj.username}, (ex) => ex? reject(new Error("such username already created")) : resolve(obj));
    })})
    .then( (obj) => {return new Promise( (resolve, reject) => {
